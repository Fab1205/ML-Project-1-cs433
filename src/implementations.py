# Same file as the one in the root
# Copied here to avoid circular imports
# Only diff is the espilon added to the log in compute_loss_logistic
...
def compute_loss_logistic(y: np.array, tx: np.array, w: np.array) -> float:
    ...
    loss = -np.mean(
        y_ * np.log(pred + epsilon) + (1 - y_) * np.log(1 - pred + epsilon)
...
def batch_iter(y: np.array, tx: np.array, batch_size, num_batches=1, shuffle=True):
    ...
    To prevent the remainder datapoints from ever being taken into account, each of the shuffled indexes is added a
    random amount
    ...
    Disclaimer: To keep this function simple, individual datapoints are not shuffled. For a more random result
    consider using a batch_size of 1.
